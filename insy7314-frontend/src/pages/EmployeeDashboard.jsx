...
const res = await axios.get("https://localhost:4000/api/employee/transactions", {
  ...
  setToast({ message: err.response?.data?.message || "Failed to fetch transactions", type: "error" });  
  setTransactions([]);  
}  
...
return ( <div className="dashboard-container"> <h1>Employee Dashboard</h1>
        ...
        transactions.map((t) => (  
          <tr key={t._id}>  
            ...
            <td className="actions">  
              <button onClick={() => handleAction(t._id, "verify")} className="approve-btn">  
                Verify  
              </button>  
              <button onClick={() => handleAction(t._id, "submit")} className="submit-btn">  
                Submit  
              </button>  
            </td>  
          </tr>  
